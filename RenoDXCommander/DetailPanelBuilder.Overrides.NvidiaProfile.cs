...
public partial class DetailPanelBuilder
{
    private void BuildNvidiaProfileSection(GameCardViewModel card, string capturedName)
    {
        ...
        var nvidiaHeaderText = Localizer.Get("DP_Nv_Header");
        ...
        if (card.HasAnyDlssStreamline)
        {
            ...
            {
                Content = Localizer.Get("DP_Nv_MultiFrameGen"),
                FontSize = 11,
                Height = 32,
            ...
            if (FeatureFlags.DlssNr)
            {
                ...
                if (!hasDlssnr)
                {
                    var presetPlaceholderLabel = new TextBlock { Text = Localizer.Get("DP_Nv_Preset"), FontSize = 10, Foreground = UIFactory.Brush(ResourceKeys.TextTertiaryBrush), Margin = new Thickness(0, 2, 0, 0), Opacity = 0 };
                ...
                {
                    Content = Localizer.Get("DP_Nv_DeployDll"),
                    FontSize = 11,
                    Height = 32,
                    HorizontalAlignment = HorizontalAlignment.Stretch,
                    Background = UIFactory.Brush(ResourceKeys.AccentBlueBgBrush),
                ...
                deployNrBtn.Click += async (s, ev) =>
                {
                    ...
                    deployNrBtn.Content = Localizer.Get("DP_Nv_Downloading");
                    ...
                    try
                    {
                        ...
                        if (isCustom)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    _window.DispatcherQueue?.TryEnqueue(() => { deployNrBtn.Content = Localizer.Get("DP_Nv_NotInCustomDlss"); deployNrBtn.IsEnabled = true; });
                                    return;
                                }
                        ...
                        else
                        {
                            ...
                            {
                                _window.DispatcherQueue?.TryEnqueue(() => { deployNrBtn.Content = Localizer.Get("DP_Nv_NotAvailable"); deployNrBtn.IsEnabled = true; });
                                return;
                            }
                            else if (isDefault)
                            {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        _window.DispatcherQueue?.TryEnqueue(() => { deployNrBtn.Content = Localizer.Get("DP_Nv_DeployDll"); deployNrBtn.IsEnabled = true; });
                    }
                };
            ...
            {
                Content = Localizer.Get("DP_Nv_RestoreDlssSl"),
                FontSize = 11,
                Height = 32,
                HorizontalAlignment = HorizontalAlignment.Stretch,
            ...
            {
                Content = Localizer.Get("DP_Nv_QuickApply"),
                FontSize = 11,
                Height = 32,
                HorizontalAlignment = HorizontalAlignment.Stretch,
